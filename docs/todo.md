...
- GitHub Actions는 첫 push 뒤 저장소의 Actions 탭에서 실제로 통과하는지 확인할 것 (로컬에서는 실행 불가)
...
- 사용자가 정해야 할 값 (백테스트 결과를 보기 전에 확정): 실전 전환 기준 4개 중 최소 거래 건수, 비용 반영 기대값, 최대 손실폭 한도는 2026-09-19에 확정해서 docs/strategy.md에 기록함. 남은 것은 코스피200과 같은 종목군 보유 대비 "통과 조건"(수익률이 같거나 높아야 하는지, 낮아도 낙폭이 확연히 작으면 인정하는지) 하나
